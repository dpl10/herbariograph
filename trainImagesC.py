...
EPSILON = 1e-07
I32 = 2147483647
LOSSES = ('bel', 'cbf', 'cbs', 'cbbs', 'ccm', 'ce', 'cos', 'fce', 'la', 'lab', 'lw', 'lwb', 'mae', 'mse', 'nis')
OPTIMIZERS = ('a', 'aw', 'sgd')
SCHEDULERS =  ('clr', )
TRAINERS = []
for loss in LOSSES:
...
settings['epochs'] = 256
settings['focalAlpha'] = 0.25
settings['focalGamma'] = 2.0
settings['gpu'] = '0'
...
try:
	arguments, values = getopt.getopt(sys.argv[1:], 'A:a:B:b:C:cd:e:F:f:G:g:hi:k:l:m:n:o:p:Qqr:s:T:t:u:v:w:xy:', ['alpa=', 'array=', 'beta=', 'batch=', 'ccm=', 'cpu', 'decay=', 'epochs=', 'focal=', 'function=', 'gamma=', 'gpu=', 'help', 'input=', 'kappa=', 'learning=', 'model=', 'numberMap=', 'output=', 'processors=', 'quantity', 'remix', 'random=', 'smooth=', 'Tau=', 'train=', 'tau=', 'validate=', 'weight=', 'ycount='])
except getopt.error as err:
	eprintWrap(str(err))
	sys.exit(2)
for argument, value in arguments:
	if argument in ('-A', '--alpha') and float(value) > 0.0:
		settings['focalAlpha'] = float(value)
	elif argument in ('-a', '--array') and int(value) > 0:
	...
	if argument in ('-F', '--focal') and float(value) > 0.0:
		settings['focalGamma'] = float(value)
		...
		eprintWrap(f"Focal cross entropy loss alpha (optional; default = {settings['focalAlpha']}): -A float | --alpha=float")
...
if settings['loss'] == 'bel':
	...
	loss = tf.keras.losses.CategoricalCrossentropy(
		from_logits = True, 
		label_smoothing = settings['labelSmoothing']
	)
elif settings['loss'] == 'cos':
	loss = cosLoss
elif settings['loss'] == 'fce':
	loss = tf.keras.losses.CategoricalFocalCrossentropy(
		alpha = settings['focalAlpha'],
		axis = -1,
		from_logits = True,
		gamma = settings['focalGamma'],
		label_smoothing = settings['labelSmoothing'],
